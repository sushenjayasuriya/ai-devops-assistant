...
@Component
public class GetKubernetesDeploymentsTool implements DevOpsTool {

    private final KubernetesIntegration kubernetesIntegration;
    private final IntegrationRepository integrationRepository;

    public GetKubernetesDeploymentsTool(KubernetesIntegration kubernetesIntegration, IntegrationRepository integrationRepository) {
    ...
    public String getName() {
        return "get_kubernetes_deployments";
    }

    @Override
    public String getDescription() {
        return "Retrieve the status of Kubernetes Deployments (desired replicas, ready replicas, updated replicas, container image).";
    }
    ...
    public ToolExecutionResult execute(Map<String, Object> parameters) {
        ...
        try {
            List<K8sDeploymentDto> deployments = kubernetesIntegration.getDeployments(configEncrypted, namespace);
            return ToolExecutionResult.ok(getName(), deployments);
        } catch (Exception ex) {
            return ToolExecutionResult.error(getName(), "Failed to query Kubernetes deployments: " + ex.getMessage());
        }
    }
}
